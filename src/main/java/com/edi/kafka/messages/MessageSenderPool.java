...
public class MessageSenderPool {
	...
	private final LinkedBlockingQueue<MessageSenderNewImpl> queue;
	 ...
	 */
	public MessageSenderPool(int size, String topic, String brokerStr, Properties properties)
	{
		...
		//this.props.put(KafkaConstants.BROKER_LIST, brokerStr);
                ...
                String brokerList = this.props.getProperty("bootstrap.servers");
	...
	private void init() 
	{
		...
		try {
			...
			LOGGER.error("Failed to init the MessageSenderPool", e);
                        System.out.println(">>>>>>>>>>Failed to init the MessageSenderPoo"+e.toString());
                        e.printStackTrace();
		}catch(Throwable t){
	...
	class InitTask implements Callable<Boolean>
	{
		...
		public Boolean call() throws Exception {
			MessageSenderNewImpl sender = new MessageSenderNewImpl(props, topic, pool);
			if(sender!=null)
			{
	 ...
	 * a sender instance
	 */
	public MessageSenderNewImpl getSender(long waitTimeMillis) throws ProducerException{
		try {
            if (!freeSender.tryAcquire(waitTimeMillis, TimeUnit.MILLISECONDS))
                throw new ProducerException("Timeout waiting for idle object in the pool.");

        } catch (InterruptedException e) {
            throw new RuntimeException("Interrupted waiting for idle object in the pool .");
        } 
		closingLock.readLock().lock();
		MessageSenderNewImpl sender = queue.poll(); 
                /*if (sender==null)
                    System.out.println("sender NULL");
                else System.out.println("sender NOT NULL");*/
		if(sender==null)
		{
                    //
                    //MODIFICA : metto la mia implementazione di Sender con 
                    // il producer di Kafka aggiornato
			sender = new MessageSenderNewImpl(props, topic, this);
			if(sender!=null)
			{
                           System.out.println("Add new sender to the pool.");
	 ...
	 */
	public void returnSender(MessageSenderNewImpl sender) {
